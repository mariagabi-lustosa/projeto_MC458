...
if __name__ == "__main__":
   filename_A = "sparse_n100_p1.csv"
   filename_B = "sparse_n100_p5.csv"


   triples_A = load_triples_from_csv(filename_A)
   triples_B = load_triples_from_csv(filename_B)


   A = convert_triples_to_paired_hash(triples_A)
   B = convert_triples_to_paired_hash(triples_B)


   print("Alguns acessos em A:")
   print("A[81,94] =", A.get_val((81, 94)))
   print("A[25,63] =", A.get_val((25, 63)))
   print("A[0,0]   =", A.get_val((0, 0)))


   # transposta
   AT = A.transpose()
   print("\nA^T[94,81] (igual a A[81,94]):", AT.get_val((94, 81)))


   # soma
   C = add_matrix(A, B)
   print("\nC = A + B. Exemplo em C[81,94]:", C.get_val((81, 94)))


   # multiplicação por escalar
   D = scalar_mul(2, A)
   print("\nD = 2 * A. Exemplo em D[81,94]:", D.get_val((81, 94)))


   # multiplicação de matrizes (A x B)
   E = matrix_mul(A, B.transpose())  # por ex., se quiser A (100x100) x B^T (100x100)
   # Não sabemos de antemão quais posições são não nulas, então só imprimimos alguns acessos:
   print("\nExemplo de E = A x B^T em [81,81]:", E.get_val((81, 81)))
